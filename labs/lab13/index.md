...
modify the types of the instance variables within the nested `Entry` class to match this change!**
...
{: .task}
>Add generic type parameters to `Entry` and modify the types of instance variables accordingly.
...
{: .task}
>First, create a constructor that initially creates **26**
empty spaces (for 26 letters in our alphabet) in the array-backed data structure, and add any necessary instance variables. Then, implement the following methods of the `Map61BL` interface to update our data structure:
...
function that works like the `String.hashCode()` example introduced [above](./#idea-one-array-based-map). -->
...
Let us establish the default size of our backing array. For this HashMap implementation, **change your default size be 16** (not 26).
...
Let's add external chaining to our initial implementation!
...
need to put in a little work to get it working with Java arrays; feel free to reference [this StackOverflow post](https://stackoverflow.com/questions/529085/how-to-create-a-generic-array-in-java)
and [this one as well](https://stackoverflow.com/questions/18581002/how-to-create-a-generic-array)
for workarounds, or look at our example below.

To create your backing array of LinkedLists, the syntax would look something like this:
```java
LinkedList<Entry<K, V>>[] buckets = (LinkedList<Entry<K, V>>[]) new LinkedList[16];
```

**Make sure you are initializing your LinkedLists in your map's constructor(s)!**

{: .task}
>Create your backing array of LinkedLists, and make changes to the following functions to support external
chaining:
